...
func (d *DB) GetCacheEntry(ctx context.Context, docID int64) (*CacheEntry, error) {
	row := d.QueryRow(ctx, `SELECT tg_doc_id, file_path, bytes, pinned, completed FROM cache_entries WHERE tg_doc_id=$1`, docID)
	c := &CacheEntry{}
...
// LRU timestamps, for reconciliation and strict capacity enforcement.
func (d *DB) AllCompletedCacheEntries(ctx context.Context) ([]CacheEntry, error) {
	rows, err := d.Query(ctx, `SELECT tg_doc_id, file_path, bytes, pinned, completed, last_accessed_at FROM cache_entries WHERE completed=true ORDER BY last_accessed_at ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var c CacheEntry
		if err := rows.Scan(&c.TGDocID, &c.FilePath, &c.Bytes, &c.Pinned, &c.Completed, &c.LastAccessedAt); err != nil {
			return nil, err
		}
		out = append(out, c)
	}
...
func (d *DB) PinByVideoID(ctx context.Context, videoID int64) (int64, bool, error) {
	row := d.QueryRow(ctx, `UPDATE cache_entries SET pinned=true WHERE tg_doc_id=(SELECT tg_doc_id FROM videos WHERE id=$1) RETURNING tg_doc_id, completed`, videoID)
	var docID int64
	var completed bool
...
func (d *DB) LRUCandidates(ctx context.Context, max int) ([]CacheEntry, error) {
	rows, err := d.Query(ctx, `SELECT tg_doc_id, file_path, bytes, pinned, completed FROM cache_entries WHERE completed=true AND pinned=false ORDER BY last_accessed_at ASC LIMIT $1`, max)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
